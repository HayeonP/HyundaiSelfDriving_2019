...
namespace vcsAgent
{
    vAgent::vAgent()
    {
        ...
        mm_estop_sub = nh.subscribe("/mission_estop",100,&vAgent::mission_mgr_estopCallback,this);
        om_estop_sub = nh.subscribe("/obj_mgr_estop",100,&vAgent::obj_mgr_estopCallback,this);
        score_sub = nh.subscribe("/ndt_stat",100,&vAgent::scoreCallback,this);
        tunnel_sub = nh.subscribe("/tunnel",100,&vAgent::tunnelCallback,this);
   ...
   void vAgent::mission_mgr_estopCallback(const std_msgs::BoolConstPtr& msg)
    {
	mmestop = msg->data;
        if(mmestop || omestop)
        {
   ...
   void vAgent::scoreCallback(const autoware_msgs::NDTStatConstPtr& msg)
   {
	ndtscore = msg->score;	
   }

   void vAgent::tunnelCallback(const std_msgs::BoolConstPtr& msg)
    {
	w_smoothing = msg->data;
    }
   void vAgent::obj_mgr_estopCallback(const std_msgs::BoolConstPtr& msg)
    {
	omestop = msg->data;
        if(mmestop || omestop)
        {
	...
	w_vari = input_msg->twist.angular.z - buffer[1];//??
	...
	if(w_smoothing && (ndtscore > score_thr) && (w_vari > w_thr))
	{
		dtos = to_string(buffer[1] + w_vari * w_ratio);
		set_ang += dtos;
		set_ang += "\n";
...
*/
    ...
    int vAgent::MainLoop()
    {
        ...
        becat = 0;
        printf("Hello! This is vcs_agent\n connected with VCS\n");
