...
GlobalPlanner::GlobalPlanner()
{
	...
	op_global_planner_ok_pub = nh.advertise<std_msgs::Bool>("/op_global_planner_ok", 1, true);
...
bool GlobalPlanner::GenerateGlobalPlan(PlannerHNS::WayPoint& startPoint, PlannerHNS::WayPoint& goalPoint, std::vector<std::vector<PlannerHNS::WayPoint> >& generatedTotalPaths)
{
	...
	if(generatedTotalPaths.size() > 0 && generatedTotalPaths.at(0).size()>0)
	{
		...
		for(unsigned int i=0; i < generatedTotalPaths.size(); i++)
		{
			...
			//ICHTHUS: Debugging topic publishing
			op_global_planner_ok_msg.data = true;
			op_global_planner_ok_pub.publish(op_global_planner_ok_msg);
		}
		return true;
	}
	else
...
void GlobalPlanner::MainLoop()
{
	...
	{
		//ICHTHUS: Debugging topic publishing
		op_global_planner_ok_msg.data = false;
		op_global_planner_ok_pub.publish(op_global_planner_ok_msg);
		ros::spinOnce();
