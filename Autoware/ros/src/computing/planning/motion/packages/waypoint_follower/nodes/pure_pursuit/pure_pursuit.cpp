...
bool PurePursuit::canGetCurvature(double *output_kappa)
{
  ...
  if (next_waypoint_number_ == -1)
  {
    ...
    ROS_WARN("can_get_curvature == False (next_waypoint_number == -1)"); // ICHTHUS
    return false;
  }
  ...
  if (!is_valid_curve)
  {
    ROS_WARN("can_get_curvature == False (all waypoints within minimum_lookahead_distance)"); // ICHTHUS
    return false;
  }
  ...
  if (!interpolation)
  {
    ...
    ROS_WARN("can_get_curvature == False (curvature interpolation fails)"); // ICHTHUS
    return false;
  }
