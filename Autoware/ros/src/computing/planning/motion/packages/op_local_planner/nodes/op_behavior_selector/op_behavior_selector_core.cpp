...
BehaviorGen::BehaviorGen()
{
	...
	op_behavior_selector_ok_pub = nh.advertise<std_msgs::Bool>("/op_behavior_selector_ok", 1, true);//ICHTHUS: Debugging topic publishing

	sub_current_pose = nh.subscribe("/current_pose", 10,	&BehaviorGen::callbackGetCurrentPose, this);
	//ICHTHUS: max velocity topic for velocity profiling
        sub_max_velocity = nh.subscribe("/mission_manager_maxvel", 1, &BehaviorGen::callbackGetMaxVelocity, this);
...
//ICHTHUS: max velocity topic for velocity profiling
void BehaviorGen::callbackGetMaxVelocity(const std_msgs::Float32ConstPtr &msg)
{
        m_PlanningParams.maxSpeed = msg->data;
        m_CarInfo.max_speed_forward = msg->data;
        m_BehaviorGenerator.ProfilingVelocity(m_PlanningParams, m_CarInfo);
	std::cout<<"op_behavior: "<< m_PlanningParams.maxSpeed<<" , m_params.max Speed"<<std::endl;
        std::cout<<"op_behavior: "<< m_CarInfo.max_speed_forward<<" m_CarInfo.max_Speed_forward"<<std::endl;
}
...
void BehaviorGen::MainLoop()
{
	...
	while (ros::ok())
	{
		...
		if(bNewCurrentPos && m_GlobalPaths.size()>0)
		{
			...
			op_behavior_selector_ok_msg.data = true;
			op_behavior_selector_ok_pub.publish(op_behavior_selector_ok_msg);
		}
		else
			sub_GlobalPlannerPaths = nh.subscribe("/lane_waypoints_array", 	1,		&BehaviorGen::callbackGetGlobalPlannerPath, 	this);
