...
static ros::Publisher ndt_matching_ok_pub;
static std_msgs::Bool ndt_matching_ok_msg;
int score_delta = 0;
int score_latest = 1;
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  if (map_loaded == 1 && init_pos_set == 1)
  {
   ...
   //ICHTHUS: Debugging topic publishing
    ...
    score_delta =fabs(score_latest - fitness_score);
    score_latest = fitness_score;
    if(fitness_score < 0.5 && score_delta < 0.1)    {
	ndt_matching_ok_msg.data = true;
    	ndt_matching_ok_pub.publish(ndt_matching_ok_msg);
    }
    else 
    {
	ndt_matching_ok_msg.data = false;
    	ndt_matching_ok_pub.publish(ndt_matching_ok_msg);
    }
...
int main(int argc, char** argv)
{
  ...
  if (nh.getParam("localizer", _localizer) == false)
  {
    ...
    ROS_ERROR("localizer is not set.");
    return 1;
  }
  else ROS_ERROR("localizer is set.");
  ...
  else ROS_ERROR("tf_pitch is set.");
  ...
  //ICHTHUS: Debugging topic publishing
  ndt_matching_ok_pub = nh.advertise<std_msgs::Bool>("/ndt_matching_ok", 1, true);
