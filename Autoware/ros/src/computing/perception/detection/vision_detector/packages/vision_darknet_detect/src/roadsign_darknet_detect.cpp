 ...
 */
#include "roadsign_darknet_detect.h"
...
namespace darknet
{
    ...
    std::vector< RoadsignClassScore<float> > Yolo3Detector::detect(image& in_darknet_image)
    {
    ...
    std::vector< RoadsignClassScore<float> > Yolo3Detector::forward(image& in_darknet_image)
    {
        ...
        std::vector< RoadsignClassScore<float> > detections;
        ...
        for (int i = 0; i < nboxes; i++)
        {
            ...
            if (class_id >= 0)
            {
                RoadsignClassScore<float> detection;
...
void Yolo3DetectorNode::convert_rect_to_image_obj(std::vector< RoadsignClassScore<float> >& in_objects, autoware_msgs::DetectedObjectArray& out_message)
{
    ...
    {
        {
            ...
            if(obj.x + obj.width > mat_image_.cols)
                obj.width = mat_image_.cols-obj.x-1;
            if(obj.y + obj.height > mat_image_.rows)
                obj.height = mat_image_.rows-obj.y-1;
...
image Yolo3DetectorNode::convert_ipl_to_image(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    cv::resize(mat_image_, resize_src, cv::Size(mat_image_.cols*resize_ratio_, mat_image_.rows*resize_ratio_), 0, 0, CV_INTER_NN);
        ...
        || network_input_height != image_height)
    {
        ...
        image_ratio_ = (double ) network_input_width /  (double)resize_src.cols;

        cv::resize(resize_src, final_mat, cv::Size(), image_ratio_, image_ratio_);
...
void Yolo3DetectorNode::image_callback(const sensor_msgs::ImageConstPtr& in_image_message)
{
    ...
    //     if(location.x+location.width > mat_image_.cols-1) location.width = mat_image_.cols-1 - location.x;
    //     if(location.y+location.height > mat_image_.rows-1) location.height = mat_image_.rows-1 - location.y;
    //     cv::Mat test = mat_image_(location);
    //     cv::imshow(std::string("test") + std::to_string(num), test);
    //     num++;
    // }  
...
void Yolo3DetectorNode::number_filtering(std::vector< RoadsignClassScore<float> >& detections){
    for(auto it = detections.begin(); it != detections.end();){        
        ...
        cv::Rect location((*it).x/image_ratio_ - image_left_right_border_/image_ratio_, 
                            (*it).y/image_ratio_ - image_top_bottom_border_/image_ratio_, 
                            (*it).w/image_ratio_, (*it).h/image_ratio_);
        if(location.x + location.width >= mat_image_.cols)
            location.width = mat_image_.cols - location.x - 1;
        if(location.y + location.height >= mat_image_.rows)
            location.height = mat_image_.rows - location.y - 1;
        if(location.x < 0){
        ...
        cv::Mat candidate = mat_image_(location);
...
void Yolo3DetectorNode::Run()
{
    ...
    if (private_node_handle.getParam("network_definition_file", network_definition_file))  // CFG
    {
    ...
    if (private_node_handle.getParam("pretrained_model_file", pretrained_model_file))  //  weights
    {
    ...
    if (private_node_handle.getParam("names_file", names_file)) // names
    {
    ...
    if(!private_node_handle.getParam("ann_path",ann_path_)){
		std::cout << "not found : "<< "ann_path" << std::endl;
	}
    number_detector_.setPath(std::string(ann_path_));
    
    private_node_handle.param<float>("resize_ratio", resize_ratio_, 1.0);
    ROS_INFO("[%s] score_threshold: %f",__APP_NAME__, resize_ratio_);
    ...
    publisher_objects_ = node_handle_.advertise<autoware_msgs::DetectedObjectArray>("/detection/image_detector/roadsigns", 1);
