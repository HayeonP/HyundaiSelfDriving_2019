...
#include <roadsign_class_score.h>
...
namespace darknet {
    ...
    private:
        ...
        std::vector<RoadsignClassScore<float> > forward(image &in_darknet_image);
    public:
        Yolo3Detector() {}
        ...
        std::vector<RoadsignClassScore<float> > detect(image &in_darknet_image);
...
class Yolo3DetectorNode {
    ...
    void                            convert_rect_to_image_obj(std::vector< RoadsignClassScore<float> >& in_objects,
    ...
    //PHY
    void                            number_filtering(std::vector< RoadsignClassScore<float> >& detections);
