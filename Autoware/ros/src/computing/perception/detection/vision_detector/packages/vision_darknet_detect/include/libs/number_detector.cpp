...
int NumberDetector::annPredict(IplImage* roi)
{
	...
	nnetwork = ml::ANN_MLP::load(network_path_);
...
NumberDetector::NumberDetector(string network_path):network_path_(network_path)
{
...
int NumberDetector::detect(Mat input)
{
	...
	Mat temp;
	input.copyTo(temp);
	IplImage *image = new IplImage(temp);
