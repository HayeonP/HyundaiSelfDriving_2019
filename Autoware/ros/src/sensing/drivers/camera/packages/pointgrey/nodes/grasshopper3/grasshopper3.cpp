...
std::vector<FlyCapture2::Camera*> cameras_;
...
unsigned int get_num_cameras(FlyCapture2::BusManager* bus_manager)
{
	...
	if (error != FlyCapture2::PGRERROR_OK)std::vector<FlyCapture2::Camera*> cameras;
	{
		//ROS_WARN("[[[[[14");
		error.PrintErrorTrace();
		std::exit(-1);
	...
	error = bus_manager->GetNumOfCameras(&cameras);
	if (error != FlyCapture2::PGRERROR_OK){
		ROS_ERROR("Cannot Get Camera");
		return -1;
	}
 ...
 * @param nh ros node handlestd::vector<FlyCapture2::Camera*> cameras;
...
int main(int argc, char **argv)
{
	...
	ros::Publisher camera_ok_pub = n.advertise<std_msgs::Bool>("/camera_ok", 10);
	std_msgs::Bool camera_ok_msg;
	camera_ok_msg.data = false;
	camera_ok_pub.publish(camera_ok_msg);
	...
	initialize_cameras(cameras_, &busMgr, camera_num, desired_mode, desired_pixel_format, timeout);
	camera_ok_msg.data = true;
	...
	ROS_ERROR("CAMERA READY!");

	start_capture(cameras_);
