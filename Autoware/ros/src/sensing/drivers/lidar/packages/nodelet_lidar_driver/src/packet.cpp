#include <nodelet_lidar_driver/packet.h>
...
int create_socket() {
  ...
  if (fcntl(sockfd, F_SETFL, O_NONBLOCK|FASYNC) < 0) { // why FASYNC ???
    perror("non-block");
    return -1;
  }
...
int read_from_socket(int sockfd, struct ip_pkt& packet, struct sockaddr_in& sender_address){
  ...
  while (true) {
    ...
    if (nbytes < 0) {
      ...
      ROS_INFO("read_from_socket() : Unknown socket SIZE");
    }
...
bool read_pcap_file(pcap_t *pcap_, struct ip_pkt& packet)  {
  ...
  for (int retry = 0; retry < 5; retry++) {
    if ((ret = pcap_next_ex(pcap_, &pcap_pkthdr, &p_data)) >= 0) {
      ...
      ROS_INFO("read_pcap_file() : %d", packet.size);
