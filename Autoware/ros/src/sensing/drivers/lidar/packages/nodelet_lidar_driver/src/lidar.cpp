#include <nodelet_lidar_driver/lidar.h>

namespace VLP16 {
  void get_VLP16_msg(bool socket_flag, ip_pkt& packet, nodelet_lidar_driver::VLP16 &vlp16_msg) {
  ...
  void print_VLP16_packet(const nodelet_lidar_driver::VLP16& msg){ //, int lidar_port) {
  ...
  bool pointInRange(float range) {
    return (range >= vlp_config.min_range
	    && range <= vlp_config.max_range);
  }
  ...
  void add_VLP16_vector_msg_to_cloud(const std::vector<nodelet_lidar_driver::VLP16::ConstPtr> &vector_msg, pcl::PointCloud<PointXYZIR>& cloud
				     ,float sin_rot_table_[], float cos_rot_table_[]){
    // VLP16_packet *p_data = (VLP16_packet *) &msg.buf; ///???
    PointXYZIR point;
    ...
    for(int i = 0; i < vector_msg.size(); i++){
	  ...
	  for(int dsr = 0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k += RAW_SCAN_SIZE) {      
            ...
            // ROS_INFO("add : calibration : %p", &calibration_);
            ...
            if ((azimuth_corrected >= vlp_config.min_angle && azimuth_corrected <= vlp_config.max_angle 
                 && vlp_config.min_angle < vlp_config.max_angle)
		||(vlp_config.min_angle > vlp_config.max_angle && (azimuth_corrected <= vlp_config.max_angle 
								   || azimuth_corrected >= vlp_config.min_angle))) {
		...
		* (1 - corrections.focal_distance / 13100);
	      ...
	      if (pointInRange(distance)) { // from rawdata.h
...
namespace OSI64 {
  void get_OSI64_msg(bool socket_flag, ip_pkt& packet, nodelet_lidar_driver::OSI64 &osi64_msg, int offset) {
    uint8_t* pkt_base;
  ...
  void print_OSI64_packet(const nodelet_lidar_driver::OSI64& msg){ //, int lidar_port) {
  ...
  void add_OSI64_msg_to_cloud(const nodelet_lidar_driver::OSI64 &msg, pcl::PointCloud<PointXYZIR>& cloud) {
