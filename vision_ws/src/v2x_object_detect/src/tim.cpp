...
TIMProcessor::TIMProcessor(ros::NodeHandle& nh, std::string combined_topic_name)
    ...
    {
    combined_msg_pub = nh.advertise<jsk_recognition_msgs::BoundingBoxArray>(
...
void TIMProcessor::findBlockedPathAndPublish(const v2x_msgs::v2x_infoConstPtr& ptr){
    ...
    */
    ...
    for(size_t i = 0 ; i < ptr->tim_nodelist_xy_latlon.size(); i += 2){
        double lat = ptr->tim_nodelist_xy_latlon[i]/10000000.0;
        double lon = ptr->tim_nodelist_xy_latlon[i+1]/10000000.0;
        ...
        ROS_INFO("[%u] : pose x y z : %lf %lf %lf",i,  pose.x, pose.y, pose.z);
        target_candidate.push_back(pose);
    }
    for(size_t i = 0 ; i < target_candidate.size(); ++i){
        ROS_WARN("target[%u] - x, y, z : %lf %lf %lf", i, 
            target_candidate[i].x, target_candidate[i].y, target_candidate[i].z);
    }

    /* find closed anchor for each point */
    std::vector<int> score(N_PATH);
    std::fill_n(score.begin(), score.size(), 0); //to avoid noise, default score is -1. I'll
...
void TIMProcessor::publish(){
    ...
    combined_msg_pub.publish(boundingBoxAry);
}
